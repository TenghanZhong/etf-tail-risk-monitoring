...
---------------------
1. Loads your multi-asset daily panel, VIX, and zero-coupon yield data.
2. Builds a quality-control layer:
   - missingness (service-time observable only)
   - invalid OHLC logic
   - extreme return jump anomalies
   - volume anomalies
   - stale-price flags
3. Builds a next-day 5% VaR monitoring model with:
   - quantile regression (HistGradientBoostingRegressor)
   - date-bootstrap ensemble uncertainty
   - OOD / regime-shift detection via PCA + Mahalanobis distance
   - recent calibration-drift score
   - explicit rolling calibration window with quantile offset
4. Builds a conservative fallback VaR:
   - combines calibrated model VaR, historical VaR, uncertainty, and quality score
5. Exports service-style outputs:
...
It also supports .xlsx / .xls if those are the actual extensions.
...
class Config:
    ...
    output_dir_name: str = "risk_monitoring_service_outputs_revised3"
    ...
    # Walk-forward setup
    train_window_days: int = 756               # approx 3 years
    calib_window_days: int = 63                # approx 3 months, now USED explicitly
    retrain_every: int = 63                    # approx quarterly retraining
...
def bootstrap_by_date(train_df: pd.DataFrame, rng: np.random.RandomState, frac_dates: float) -> pd.DataFrame:
    ...
    if tmp.empty:
        return train_df.sample(frac=1.0, replace=True, random_state=int(rng.randint(1_000_000))).reset_index(drop=True)
...
def add_quality_layer(df: pd.DataFrame, cfg: Config) -> pd.DataFrame:
    ...
    # Service-time observable only. Do NOT use ret_next here.
...
def build_ood_feature_columns(feature_cols: List[str]) -> List[str]:
    # Do not let numeric symbol code create fake cross-asset geometry in PCA/Mahalanobis.
...
def recent_drift_score(results_so_far: pd.DataFrame, symbol: str, current_date: pd.Timestamp, cfg: Config) -> float:
    ...
    # 用最终服务输出 breach_safe，而不是 breach_model
    rate = float(past["breach_safe"].mean())

    # 只惩罚 under-coverage，不惩罚过保守
    excess = max(rate - cfg.alpha, 0.0)

    # 让 drift 更平滑，不要太容易顶到 1
...
def build_dataset(cfg: Config) -> pd.DataFrame:
    ...
    feature_cols = build_feature_columns(panel)
    ...
    # Keep rows even if ret_next is missing, so the latest service snapshot remains service-valid.
    return panel
...
def run_backtest(df: pd.DataFrame, cfg: Config) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for current_ts in tqdm(test_dates, desc="Walk-forward backtest", ncols=100):
        ...
        if need_retrain:
            ...
            for k in range(cfg.n_ensemble):
                ...
                train_pred_cols.append(model.predict(prepare_X(train_df, feature_cols, fill_values)))
        ...
        X_pred_ood = prepare_X(pred_df, retrain_state["ood_feature_cols"], retrain_state["fill_values"][retrain_state["ood_feature_cols"]])
        ...
        conservative_adj = adj_scale * (
            ...
            + cfg.fallback_lambda_q * pred_df["quality_score"].fillna(0.0)
        )
        ...
        pred_df["fallback_ratio"] = (pred_df["fallback_size"] / adj_scale.replace(0, np.nan)).fillna(0.0).clip(0.0, 1.0)
        ...
        keep_cols = [
            "date", "symbol", cfg.target_col, "is_evaluable",
    ...
    if not rows_out:
        raise RuntimeError("Backtest produced no predictions. Check config/test_start/data coverage.")
...
def main():
    ...
    results, summary = run_backtest(panel, cfg)
    out_dir = save_outputs(results, summary, cfg, panel)

    print("=" * 90)
    print("Quality-Aware and Uncertainty-Aware ETF Risk Monitoring as a Big Data Service")
    print("=" * 90)
