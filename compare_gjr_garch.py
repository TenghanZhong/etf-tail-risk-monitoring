...
Extended version with fallback/component experiments, paper-aligned
uncertainty diagnostics, and GJR-GARCH(1,1)-t VaR baseline.
...
7. **NEW**: Adds a GJR-GARCH(1,1)-t VaR baseline, estimated per-symbol on
...
class Config:
    ...
    output_dir_name: str = "comparision"
    ...
    # GJR-GARCH baseline
    gjr_garch_min_obs: int = 200
    gjr_garch_max_iter: int = 500
    ...
    save_quality_validation_daily_outputs: bool = False
...
# ============================================================
# GJR-GARCH(1,1)-t implementation
# ============================================================

def _gjr_garch_negloglik(params: np.ndarray, returns: np.ndarray) -> float:
    """
    Negative log-likelihood for GJR-GARCH(1,1) with Student-t innovations.

    Variance equation:
        sigma2_t = omega + alpha * eps_{t-1}^2 + gamma * eps_{t-1}^2 * I(eps_{t-1}<0) + beta * sigma2_{t-1}

    Parameters vector: [mu, omega, alpha, gamma, beta, nu]
        mu    : conditional mean
        omega : variance intercept (>0)
        alpha : ARCH coefficient (>=0)
        gamma : asymmetric (leverage) coefficient (>=0)
    ...
    if omega <= 1e-10 or alpha < 0 or gamma < 0 or beta < 0 or nu <= 2.01:
        return 1e10
    ...
    # Initialise variance at sample variance
    sigma2 = np.empty(T)
    sigma2[0] = np.var(eps)
    if sigma2[0] < 1e-12:
        sigma2[0] = 1e-6
    ...
    for t_idx in range(1, T):
        leverage = 1.0 if eps[t_idx - 1] < 0.0 else 0.0
        sigma2[t_idx] = (
            omega
            + alpha * eps[t_idx - 1] ** 2
            + gamma * eps[t_idx - 1] ** 2 * leverage
            + beta * sigma2[t_idx - 1]
        )
        if sigma2[t_idx] < 1e-12:
            sigma2[t_idx] = 1e-12
    ...
    for t_idx in range(T):
        s2 = sigma2[t_idx]
        z2 = eps[t_idx] ** 2 / s2
        nll -= const - 0.5 * np.log(s2) - 0.5 * (nu + 1.0) * np.log(1.0 + z2 / (nu - 2.0))
...
def _gjr_garch_negloglik_fast(params: np.ndarray, returns: np.ndarray) -> float:
    """
    Vectorised-inner-loop version of the GJR-GARCH(1,1)-t negative log-likelihood.
    Falls back to the scalar loop for the variance recursion (unavoidable), but
    vectorises the log-likelihood summation.
    """
    mu, omega, alpha, gamma_coef, beta, nu = params

    if omega <= 1e-10 or alpha < 0 or gamma_coef < 0 or beta < 0 or nu <= 2.01:
        return 1e10
    if alpha + gamma_coef / 2.0 + beta >= 1.0:
        return 1e10
    ...
    sigma2 = np.empty(T)
    var_init = np.var(eps)
    sigma2[0] = max(var_init, 1e-12)

    for i in range(1, T):
        e_prev = eps[i - 1]
        e2 = e_prev * e_prev
        lev = gamma_coef * e2 if e_prev < 0.0 else 0.0
        sigma2[i] = omega + alpha * e2 + lev + beta * sigma2[i - 1]
        if sigma2[i] < 1e-12:
            sigma2[i] = 1e-12
    ...
    const = gammaln((nu + 1.0) / 2.0) - gammaln(nu / 2.0) - 0.5 * np.log(np.pi * (nu - 2.0))
    z2 = eps ** 2 / sigma2
    ll = const - 0.5 * np.log(sigma2) - 0.5 * (nu + 1.0) * np.log(1.0 + z2 / (nu - 2.0))
    nll = -np.sum(ll)
...
def fit_gjr_garch(
    ...
    """
    Fit GJR-GARCH(1,1)-t by MLE using scipy.optimize.minimize (Nelder-Mead).

    Returns dict with keys: mu, omega, alpha, gamma, beta, nu, sigma2_last, eps_last
    ...
    # Starting values: mu, omega, alpha, gamma, beta, nu
    x0 = np.array([
        sample_mean,
    ...
    starts = [
        x0,
        np.array([sample_mean, sample_var * 0.02, 0.03, 0.08, 0.88, 6.0]),
        np.array([sample_mean, sample_var * 0.10, 0.08, 0.03, 0.80, 10.0]),
        np.array([0.0, sample_var * 0.05, 0.05, 0.10, 0.82, 5.0]),
    ]

    for s in starts:
        try:
            res = minimize(
                _gjr_garch_negloglik_fast,
                s,
                args=(returns,),
                method="Nelder-Mead",
    ...
    mu, omega, alpha, gamma_coef, beta, nu = best_result.x

    # Enforce constraints on the solution
    if omega <= 0 or alpha < 0 or gamma_coef < 0 or beta < 0 or nu <= 2.01:
        return None
    if alpha + gamma_coef / 2.0 + beta >= 1.0:
        return None

    # Reconstruct final sigma2 for one-step-ahead forecast
    eps = returns - mu
    T = len(returns)
    sigma2 = np.empty(T)
    sigma2[0] = max(np.var(eps), 1e-12)
    for i in range(1, T):
        e_prev = eps[i - 1]
        e2 = e_prev * e_prev
        lev = gamma_coef * e2 if e_prev < 0.0 else 0.0
...
def gjr_garch_one_step_var(
    params: Dict[str, float],
    alpha_level: float = 0.05,
) -> float:
    """
    Compute one-step-ahead VaR from fitted GJR-GARCH(1,1)-t parameters.

    sigma2_{T+1} = omega + alpha * eps_T^2 + gamma * eps_T^2 * I(eps_T<0) + beta * sigma2_T
    VaR_{T+1}    = mu + sqrt(sigma2_{T+1}) * t_inv(alpha, nu) * sqrt((nu-2)/nu)

    The scaling factor sqrt((nu-2)/nu) standardises the t-distribution to
    match the GARCH variance parametrisation (unit conditional variance).
    """
    omega = params["omega"]
    a = params["alpha"]
    gamma_coef = params["gamma"]
    beta = params["beta"]
    mu = params["mu"]
    nu = params["nu"]
    ...
    e2 = eps_last ** 2
    leverage = gamma_coef * e2 if eps_last < 0.0 else 0.0
    sigma2_next = omega + a * e2 + leverage + beta * sigma2_last
    sigma_next = np.sqrt(max(sigma2_next, 1e-12))

    # Student-t quantile, standardised to unit variance
    t_q = tdist.ppf(alpha_level, nu)
    # The standardised t has variance nu/(nu-2), so scale accordingly
    scale = np.sqrt((nu - 2.0) / nu) if nu > 2.0 else 1.0
    var_estimate = mu + sigma_next * t_q * scale

    return float(var_estimate)


def gjr_garch_rolling_update(
    params: Dict[str, float],
    new_return: float,
) -> Dict[str, float]:
    """
    Given existing GJR-GARCH parameters and a new observed return,
    update sigma2_last and eps_last for the next one-step-ahead forecast
    WITHOUT re-estimating the parameters. Used between retrain points.
    """
    mu = params["mu"]
    omega = params["omega"]
    a = params["alpha"]
    ...
    eps_new = new_return - mu
    e2 = eps_new ** 2
    leverage = gamma_coef * e2 if eps_new < 0.0 else 0.0
    sigma2_new = omega + a * e2 + leverage + beta * params["sigma2_last"]
    sigma2_new = max(sigma2_new, 1e-12)

    updated = dict(params)
    updated["sigma2_last"] = float(sigma2_new)
    updated["eps_last"] = float(eps_new)
    return updated
...
# ============================================================
# GJR-GARCH management for the walk-forward loop
# ============================================================

def fit_gjr_garch_for_all_symbols(
    df: pd.DataFrame,
    train_dates: pd.DatetimeIndex,
    cfg: Config,
) -> Dict[str, Optional[Dict[str, float]]]:
    """
    Fit GJR-GARCH(1,1)-t per symbol on the training window returns.
    Returns a dict mapping symbol -> fitted params (or None if fit fails).
    """
    garch_params: Dict[str, Optional[Dict[str, float]]] = {}
    train_data = df[df["date"].isin(train_dates)].copy()

    for sym, sub in train_data.groupby("symbol"):
        returns = sub["ret"].dropna().to_numpy()
        if len(returns) < cfg.gjr_garch_min_obs:
            garch_params[str(sym)] = None
            continue
        params = fit_gjr_garch(returns, max_iter=cfg.gjr_garch_max_iter)
        garch_params[str(sym)] = params

    return garch_params


def update_garch_states_with_new_returns(
    garch_params: Dict[str, Optional[Dict[str, float]]],
    df: pd.DataFrame,
    update_dates: pd.DatetimeIndex,
) -> Dict[str, Optional[Dict[str, float]]]:
    """
    Roll the GARCH state forward through the calibration window dates
    (and any gap dates) so that at prediction time sigma2_last and eps_last
    ...
    for sym, params in garch_params.items():
        ...
        sub = df[(df["symbol"] == sym) & (df["date"].isin(update_dates))].sort_values("date")
        p = dict(params)
        for _, row in sub.iterrows():
            r = row.get("ret", np.nan)
            if np.isfinite(r):
                p = gjr_garch_rolling_update(p, float(r))
        updated[sym] = p
...
def run_backtest(
    ...
    # ---- GJR-GARCH state ----
    garch_state: Dict[str, Optional[Dict[str, float]]] = {}
    ...
    for current_ts in tqdm(test_dates, desc=f"Backtest[{experiment_name}]", ncols=100):
        ...
        if need_retrain:
            ...
            train_dates_range = pd.to_datetime(dates[train_start_idx:train_end_idx + 1])
            calib_dates_range = pd.to_datetime(dates[calib_start_idx:calib_end_idx + 1])

            train_df = df[df["date"].isin(train_dates_range)].copy()
            train_df = train_df.dropna(subset=[cfg.target_col]).copy()

            calib_df = df[df["date"].isin(calib_dates_range)].copy()
            ...
            # ---- Fit GJR-GARCH per symbol on the training window ----
            garch_state = fit_gjr_garch_for_all_symbols(df, train_dates_range, cfg)
            # Roll state forward through calibration window
            garch_state = update_garch_states_with_new_returns(garch_state, df, calib_dates_range)
            ...
            retrain_state = {
                ...
                "train_start": str(train_dates_range.min().date()),
                "train_end": str(train_dates_range.max().date()),
                "calib_start": str(calib_dates_range.min().date()),
                "calib_end": str(calib_dates_range.max().date()),
            }
        ...
        # ---- GJR-GARCH VaR for each row ----
        gjr_var_values = []
        for _, row in pred_df.iterrows():
            sym = str(row["symbol"])
            gp = garch_state.get(sym, None)
            if gp is not None:
                gjr_var_values.append(gjr_garch_one_step_var(gp, cfg.alpha))
            else:
                # Fallback: use EWMA-normal VaR if GARCH fit failed
                gjr_var_values.append(np.nan)
        pred_df["gjr_garch_var"] = gjr_var_values

        # ---- Update GARCH state with today's returns for next day ----
        for _, row in pred_df.iterrows():
            sym = str(row["symbol"])
            gp = garch_state.get(sym, None)
            if gp is not None:
                r_today = row.get("ret", np.nan)
                if np.isfinite(r_today):
                    garch_state[sym] = gjr_garch_rolling_update(gp, float(r_today))
        ...
        # ---- GJR-GARCH breach ----
        pred_df["breach_gjr_garch"] = np.where(
            (pred_df["is_evaluable"] == 1) & pred_df["gjr_garch_var"].notna(),
            (pred_df[cfg.target_col] < pred_df["gjr_garch_var"]).astype(float),
            np.nan,
        )
        ...
        keep_cols = [
            ...
            "raw_model_pred", "simple_fallback_pred", "uncertainty_only_pred", "quality_only_pred", "full_fallback_pred",
            "safe_hist_anchor", "safe_hist_var", "hist_var_alpha", "ewma_norm_var",
            "gjr_garch_var", "breach_gjr_garch",
            ...
            "use_quality_feature", "use_simple_hist_fallback",
...
def evaluate_results(results: pd.DataFrame, cfg: Config) -> pd.DataFrame:
    ...
    # ---- GJR-GARCH VaR evaluation ----
    if "gjr_garch_var" in eval_results.columns:
        gjr_mask = eval_results["gjr_garch_var"].notna()
        if gjr_mask.sum() > 0:
            gjr_sub = eval_results[gjr_mask]
            gjr_breach = gjr_sub["breach_gjr_garch"].to_numpy()
            add_eval(
                "overall_gjr_garch_var",
                gjr_sub[cfg.target_col].to_numpy(),
                gjr_sub["gjr_garch_var"].to_numpy(),
                gjr_breach,
            )
    ...
    if "vix" in eval_results.columns and eval_results["vix"].notna().any():
        ...
        if stress.sum() > 0:
            ...
            # ---- GJR-GARCH stress evaluation ----
            if "gjr_garch_var" in sub.columns:
                gjr_stress_mask = sub["gjr_garch_var"].notna()
                if gjr_stress_mask.sum() > 0:
                    gjr_stress_sub = sub[gjr_stress_mask]
                    gjr_stress_breach = gjr_stress_sub["breach_gjr_garch"].to_numpy()
                    add_eval(
                        "stress_gjr_garch_var",
                        gjr_stress_sub[cfg.target_col].to_numpy(),
                        gjr_stress_sub["gjr_garch_var"].to_numpy(),
                        gjr_stress_breach,
                    )
    ...
    for sym, sub in eval_results.groupby("symbol"):
        ...
        # ---- Per-symbol GJR-GARCH ----
        if "gjr_garch_var" in sub.columns:
            gjr_sym_mask = sub["gjr_garch_var"].notna()
            if gjr_sym_mask.sum() > 0:
                gjr_sym_sub = sub[gjr_sym_mask]
                gjr_sym_breach = gjr_sym_sub["breach_gjr_garch"].to_numpy()
                add_eval(
                    f"{sym}_gjr_garch_var",
                    gjr_sym_sub[cfg.target_col].to_numpy(),
                    gjr_sym_sub["gjr_garch_var"].to_numpy(),
                    gjr_sym_breach,
                )
...
def run_quality_validation_experiment(
    ...
    no_q_fallback_results, no_q_fallback_summary = run_backtest(
        panel,
        cfg,
    ...
    summary_frames.append(no_q_fallback_summary)
    if cfg.save_quality_validation_daily_outputs:
        daily_frames.append(no_q_fallback_results)
...
def main():
    ...
    # ---- Print GJR-GARCH summary explicitly ----
    gjr_rows = summary[summary["segment"].str.contains("gjr_garch", case=False)]
    if not gjr_rows.empty:
        print("-" * 100)
        print("GJR-GARCH(1,1)-t VaR BASELINE RESULTS:")
        print(gjr_rows[["segment", "n", "breach_rate", "kupiec_lr_uc", "pinball_loss"]].to_string(index=False))
